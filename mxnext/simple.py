...
def leaky_relu(data, name=None, slope=0.1):
    """
    the linear rectifier activation function, y = max(x, 0)
    :param data: input symbol object
    :param name: output symbol name
    ...
    if name is None:
        prev_name = data.name
        if prev_name.endswith("_bn") or prev_name.endswith("_gn"):
            name = prev_name[:-3] + "_lrelu"
        else:
            name = prev_name + "_lrelu"

    return mx.sym.LeakyReLU(data, slope=slope, name=name)
...
def convleakyrelu(data, name, filter, kernel=1, stride=1, pad=None, dilate=1, num_group=1, no_bias=True, init=None,
             lr_mult=1.0, wd_mult=1.0, weight=None, bias=None, slope=0.1):
    d1 = conv(data, name, filter, kernel, stride, pad, dilate, num_group, no_bias, init, lr_mult, wd_mult, weight, bias)
    d2 = leaky_relu(d1, name + "_lrelu", slope)
    return d2